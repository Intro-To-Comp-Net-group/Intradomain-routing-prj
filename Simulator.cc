    ...
    if (line.find('[') != string::npos)// find configuration title
    {
        ...
        if (line.find("links") != string::npos) {
            while (getline(fstream, line)) {
                ...
                Link *lnk = new Link(node_table[link_from], node_table[link_to], (unsigned int) (delay * 1000), prob,
                                     cost);
        ...
        if (line.find("events") != string::npos) {
            ...
            while (getline(fstream, line)) {
                ...
                } else if ((pos = line.find("xmit")) != string::npos) //xmit
                {
                    ...
                    pkt->src = htons(src);
                    pkt->dst = htons(dst);
                    e = new Event_Xmit_Pkt_End_To_End_Generic(node_table[src], node_table[dst], pkt,
                                                              sizeof(struct sim_pkt_header), (unsigned int) time);
